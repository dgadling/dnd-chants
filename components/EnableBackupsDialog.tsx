    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className={`border rounded-xl p-5 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto ${isLight ? "bg-white border-zinc-200" : "bg-zinc-800 border-zinc-700"}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`space-y-3 text-[13px] leading-relaxed ${isLight ? "text-zinc-700" : "text-zinc-300"}`}>
          ...
          <p className={`text-[12px] ${isLight ? "text-blue-700" : "text-amber-200/90"}`}>If you forget the PIN below, and lose access to your logged in devices, you lose your backup. There is no recovery without the PIN.</p>
        ...
        <div className="mt-4 space-y-3">
          <div><label className={`text-[11px] uppercase tracking-wide font-semibold ${isLight ? "text-zinc-600" : "text-zinc-400"}`}>6-digit PIN</label><div className="mt-1 flex gap-2"><input type={showPin ? "text" : "password"} inputMode="numeric" pattern="\d*" maxLength={6} value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="123456" className={`flex-1 h-10 rounded-lg border px-3 text-sm tracking-widest placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus-ring-accent ${isLight ? "border-zinc-300 bg-white text-zinc-900" : "border-zinc-700 bg-zinc-900 text-zinc-100"}`} /><button type="button" onClick={() => setShowPin(v => !v)} className={`h-10 px-3 rounded-lg text-xs ${isLight ? "bg-zinc-100 text-zinc-700 hover:bg-zinc-200" : "bg-zinc-700 text-zinc-300 hover:bg-zinc-600"}`}>{showPin ? "Hide" : "Show"}</button></div>{pin && !isValidPin ? <div className={`text-[11px] mt-1 ${isLight ? "text-blue-700" : "text-amber-300"}`}>PIN must be exactly 6 digits</div> : null}</div>
          <div><label className={`text-[11px] uppercase tracking-wide font-semibold ${isLight ? "text-zinc-600" : "text-zinc-400"}`}>Confirm PIN</label><input type={showPin ? "text" : "password"} inputMode="numeric" pattern="\d*" maxLength={6} value={confirmPin} onChange={(e) => setConfirmPin(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="123456" className={`mt-1 w-full h-10 rounded-lg border px-3 text-sm tracking-widest placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus-ring-accent ${isLight ? "border-zinc-300 bg-white text-zinc-900" : "border-zinc-700 bg-zinc-900 text-zinc-100"}`} />{confirmPin && pin !== confirmPin ? <div className="text-[11px] text-red-300 mt-1">PINs do not match</div> : null}</div>
