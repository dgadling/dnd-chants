  ...
  async (req, res) => {
    ...
    try {
      ...
      const html = `<!doctype html><html><head><meta charset="utf-8"><title>Discord auth success</title></head><body style="font-family:system-ui;padding:20px;background:#18181b;color:#e4e4e7"><p>Discord login successful – you can close this window.</p><script>(function(){var payload={type:"discord-auth-success",customToken:${safeJson(
        ...
        state
      )}};try{if(window.opener){var targetOrigin=window.location.origin;var allowed=["https://chants-506202.web.app","https://chants-506202.firebaseapp.com",targetOrigin];var openerOrigin="";try{openerOrigin=window.opener.location.origin;}catch(e){}if(openerOrigin&&allowed.indexOf(openerOrigin)===-1)allowed.push(openerOrigin);try{if(openerOrigin)window.opener.postMessage(payload,openerOrigin);}catch(e){}try{window.opener.postMessage(payload,"*");}catch(e){}try{window.opener.postMessage(payload,targetOrigin);}catch(e){}for(var i=0;i<allowed.length;i++){try{if(allowed[i]!==targetOrigin&&allowed[i]!==openerOrigin) window.opener.postMessage(payload,allowed[i]);}catch(e){}}}try{localStorage.setItem("dnd-chant-discord-callback",JSON.stringify(payload));}catch(e){}setTimeout(function(){window.close();},500);setTimeout(function(){if(!window.opener){window.location.href="/?discord_auth=success";}},800);}catch(e){document.body.innerHTML+="<pre>"+String(e)+"</pre>";}})();</script></body></html>`;
      res.set("Content-Type", "text/html").status(200).send(html);
    } catch (e: any) {
